...
// priors
 ...
 for(w in 1:nworked)
   target += 
    ordered_logistic_lpmf(hint[w]|probEff[prob[w]][1]+effHint,cHint)+
    ordered_logistic_lpmf(err[w]|probEff[prob[w]][2]+effErr,cErr)+
    normal_lpdf(ltime[w]| probEff[prob[w]][3]+meanTime,sigTime);
